...
while running:
    ...
    if grid.game_over and not grid.tie_game:
        ...
        won_surface = winner_font.render(f"{grid.winner_letter} Nyertél", False, color)
        window.blit(won_surface, (220, 650))

        restart_surface = restart_font.render("Nyomj a szóközre az újrakezdéshez!", False, color)
        window.blit(restart_surface, (150, 720))

    if grid.tie_game:
        tie_game_surface = winner_font.render("Döntetlen!", False, (255, 255, 255))
        window.blit(tie_game_surface, (180, 650))

        restart_surface = restart_font.render("Nyomj a szóközre az újrakezdéshez!", False, (255, 255, 255))
        window.blit(restart_surface, (150, 720))
